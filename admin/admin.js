...
async function galleryPage() {
  ...
  const render = () => {
    body.innerHTML = `<div class="friendly-tabs">${galleryGroups.map((group) => `<button class="friendly-tab ${active === group.code ? "active" : ""}" data-tab="${group.code}" type="button">${group.name}<span>${items.filter((item) => groupFor(item.category).code === group.code).length}</span></button>`).join("")}</div><div class="toolbar page-actions"><button class="btn primary" id="addGallery" type="button">+ Thêm ảnh vào ${galleryGroups.find((g) => g.code === active).name}</button><button class="btn ${dirty ? "primary" : ""}" id="saveGalleryOrder" type="button" ${dirty ? "" : "disabled"}>Lưu thứ tự</button><button class="btn" id="cancelGalleryOrder" type="button" ${dirty ? "" : "disabled"}>Huỷ thay đổi</button></div><div class="admin-card-grid gallery-card-grid" id="galleryCards">${itemsFor().map((item, index) => `<article class="admin-card gallery-card" draggable="true" data-id="${item.id}"><div class="drag-handle">Kéo thả</div><img src="${esc(item.image_url || "/ben-chill-logo.png")}" alt=""><div class="admin-card-body"><b>${esc(item.caption || "Chưa có caption")}</b><p class="mini">${item.is_visible ? "Hiển thị" : "Đang ẩn"} · Vị trí ${index + 1}</p><div class="toolbar"><button class="btn edit-gallery" type="button">Sửa</button><button class="btn toggle-gallery" type="button">${item.is_visible ? "Ẩn" : "Hiện"}</button><button class="btn danger delete-gallery" type="button">Xoá</button></div><div class="toolbar mobile-order"><button class="btn move-gallery" data-move="-1" type="button">Lên</button><button class="btn move-gallery" data-move="1" type="button">Xuống</button><button class="btn move-gallery" data-move="first" type="button">Lên đầu</button><button class="btn move-gallery" data-move="last" type="button">Xuống cuối</button></div></div></article>`).join("") || `<div class="empty-card"><h2>Chưa có ảnh</h2><p class="muted">Bấm “Thêm ảnh” để bắt đầu.</p></div>`}</div>`;
  };
