...
async function contentPage() {
  ...
  body.innerHTML = `<div class="content-manager">
    <div class="content-helper">
      <div><b>Cách dùng nhanh</b><p class="muted">Chọn đúng khối, sửa tiêu đề hoặc mô tả, đổi ảnh nếu cần rồi bấm lưu. Phần nâng cao chỉ mở khi cần chỉnh dữ liệu đặc biệt.</p></div>
      <a class="btn" href="/" target="_blank" rel="noopener noreferrer">Xem website</a>
    </div>
    <div class="item-list content-section-list">${data.map((row) => {
      const title = sectionNames[row.section_key] || row.section_key || "Section";
      return `
    <form class="panel section-form content-card" data-id="${row.id}">
      <div class="content-card-head">
        <div><span class="content-kicker">${esc(row.section_key || "")}</span><h2>${esc(title)}</h2></div>
      ...
      <div class="content-editor-grid">
        ...
        <div class="content-image-box">
          ...
          <div class="content-preview ${row.image_url ? "" : "empty"}">
            ${row.image_url ? `<img src="${esc(row.image_url)}" alt="${esc(row.image_alt || title)}">` : `<span>Chưa chọn ảnh</span>`}
          </div>
          ...
          <div class="toolbar content-image-actions">
            <button class="btn section-pick-image" type="button">Chọn từ thư viện</button>
            <button class="btn section-upload-image" type="button">Upload ảnh mới</button>
            <button class="btn danger section-clear-image" type="button">Xoá ảnh</button>
          </div>
          <label>Alt ảnh<input name="image_alt" value="${esc(row.image_alt || "")}" placeholder="Mô tả ảnh cho Google và người dùng"></label>
        </div>
      </div>
      <details class="advanced-box">
        <summary>Nâng cao: URL ảnh thủ công và dữ liệu phụ</summary>
        <div class="grid">
          <label class="full">URL ảnh thủ công<input class="manual-image-url" value="${esc(row.image_url || "")}" placeholder="https://..."></label>
          <label class="full">Dữ liệu phụ<textarea name="content_json">${esc(JSON.stringify(row.content_json || {}, null, 2))}</textarea></label>
        </div>
      </details>
    ...
    <div class="panel revision-panel">
      ...
      <p class="muted">Dùng khi cần quay lại nội dung trước lần lưu gần nhất.</p>
  ...
  $$(".manual-image-url").forEach((input) => input.addEventListener("change", () => setSectionImage(input.closest(".section-form"), input.value.trim())));
  $$(".section-form").forEach((form) => {
    form.addEventListener("click", async (event) => {
      ...
      if (event.target.classList.contains("section-pick-image")) {
        ...
        if (!names.length) return toast("Chưa có ảnh trong thư viện.", false);
    ...
    $(".section-upload-input", form).addEventListener("change", async (event) => {
      ...
      if (error) return toast(error.message, false);
      const { data: uploaded } = supa.storage.from(bucket).getPublicUrl(safeName);
      setSectionImage(form, uploaded.publicUrl);
      toast("Đã upload và gắn ảnh vào section");
    });
    form.addEventListener("submit", async (event) => {
    event.preventDefault();
